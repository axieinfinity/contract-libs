...
/**
 * @title LibRONTransferHelperExtended
 */
library LibRONTransferHelperExtended {
  /**
  ...
  function safeUnwrapTokenAndTransfer(IWRON wron, address token, address payable to, uint256 amount) internal {
    if (token == address(wron)) {
      ...
      LibNativeTransfer.safeTransfer(to, amount);
      return;
    }

    LibTransferHelper.safeTransfer(token, to, amount);
  }
  ...
  {
    if (token == address(wron)) {
      LibTransferFromHelper.safeTransferFrom(token, from, address(this), amount);
      IWRON(wron).withdraw(amount);
      LibNativeTransfer.safeTransfer(to, amount);
      return;
    }

    LibTransferFromHelper.safeTransferFrom(token, from, to, amount);
  }
